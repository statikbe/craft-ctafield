...
class PluginFieldMigration extends PluginMigration
{
    ...
    public static function getDefaultFieldLayout(bool $includeText = true, bool $enableTitle = true, bool $enableAriaLabel = false, bool $allowClass = true): FieldLayout
    {
        ...
        $linkClassesField = null;
        if ($allowClass) {
            $linkClassesField = new CustomField(Craft::$app->getFields()->getFieldByHandle("linkClasses"));
        }

        $tab1->setElements(array_filter([$linkField, $linkTextField, $linkClassesField ]));
        ...
        $tab2->setElements(array_filter([$linkTitleField, $classesField]));
